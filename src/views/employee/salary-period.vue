...
const salaryPeriod = computed<SalaryPeriodReduced | undefined>(() => {
  ...
  const salariesFindForCurrUser = salaryPeriodCustom.salaries.find(e => e.user_id === user.user?.user_id)

  if (salariesFindForCurrUser) {
    salaryPeriodCustom.salaries = [salariesFindForCurrUser]
  } else {
    salaryPeriodCustom.salaries = []
  }
  return salaryPeriodCustom
})

const back = () => {
  router.push({ name: 'employee-salary' })
}
